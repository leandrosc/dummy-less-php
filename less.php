...
class lessc {
    ...
    function multiplyTags($parents, $current) {
        ...
        foreach ($parents as $ptag) {
            foreach ($current as $tag) {
                $tags[] = trim($ptag .
                        ($tag{0} == $this->selfSelector || $tag{0} == ':' ? ltrim($tag, $this->selfSelector) : ' ' . $tag));
            }
        }
    ...
    function lib_rgbahex($color) {
        if ($color[0] != 'color')
            throw new exception("color esperou por valor rgbahex");

        return sprintf("#%02x%02x%02x%02x", isset($color[4]) ? $color[4] * 255 : 0, $color[1], $color[2], $color[3]);
    }
    ...
    function op_color_color($op, $left, $right) {
        ...
        foreach (range(1, $max - 1) as $i) {
            ...
            switch ($op) {
                ...
                case '/':
                    if ($rval == 0)
                        throw new exception("evaluate error: nao se pode dividir por zero!");
                    $out[] = $lval / $rval;
                    break;
                default:
    ...
    function parse($str = null, $initial_variables = null) {
        ...
        if ($initial_variables)
            $this->injectVariables($initial_variables);
    ...
    function throwParseError($msg = 'parse error') {
        ...
        if ($this->peek("(.*?)(\n|$)", $m))
            throw new exception($msg . ': falhou em `' . $m[1] . '` ' . $loc);
    }
    ...
    function __construct($fname = null, $opts = null) {
        ...
        if ($fname) {
            if (!is_file($fname)) {
                throw new Exception('load error: Nao foi possivel encontrar ' . $fname);
            }
            $pi = pathinfo($fname);
...
try {
    if ($handle = opendir('./less')) {
        while (false !== ($file = readdir($handle))) {
            if ($file != "." && $file != ".." && strpos($file, '_') !== 0) {
                $file = str_replace('.less', '', $file);
                lessc::ccompile("./less/$file.less", "./css/$file.css");
                echo "$file.less compilado.<br>";
            }
        }
        closedir($handle);
    }
